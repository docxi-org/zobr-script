...
**Реализовано.** 241 тест, 29 MCP tools, typecheck clean.
Срезы 1–19 завершены (бэкенд + фронтенд + REST API + i18n + per-agent roles + explicit conclude + MCP OAuth + agent-side retrieve + mobile adaptation + monolithic guide + agent experience + A2 MCP Apps виджеты + artifact infra + fullscreen A2 dashboard).
A1 sidebar dashboard отменён и вычищен (CSP sandbox claude.ai блокирует connect-src). Заменён fullscreen A2 виджетом.
...
29 MCP tools: 6 lifecycle + 2 commit/check + 2 trace records + 3 discovery + 4 CRUD +
8 store + 1 register + 1 abort + 1 guide + 1 retrieve

### MCP Apps (ext-apps) — A2 виджеты

9 виджетов рендерятся в чате (claude.ai, ChatGPT, VS Code) при вызове tools:
...
- `zs_conclude` → **Dashboard** (inline: coverage donut + result; fullscreen: timeline + events + code)

`zs_conclude` виджет поддерживает `availableDisplayModes: ["inline", "fullscreen"]`.
Сервер включает `_trace: { events, script_ref, code_snapshot }` в ответ `zs_conclude`.
...
### Artifact инфраструктура (серверная, для SPA)
...
- A1 sidebar и `zs_dashboard` tool удалены (CSP блокирует connect-src в sandbox claude.ai)
- Fullscreen A2 dashboard заменил A1 — данные через MCP, CSP не мешает
