...
Срезы 1–16 завершены (бэкенд + фронтенд + REST API + i18n + per-agent roles + explicit conclude + MCP OAuth + agent-side retrieve + mobile adaptation + monolithic guide + agent experience improvements).
...
- `TODO.md` — срезы 15–16 завершены (monolithic guide, agent experience).
...
- `COMPROMISES.md` — реестр технического долга (пуст).
