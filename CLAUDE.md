...
**Реализовано.** 242 теста, 27 MCP tools, typecheck clean.
Срезы 1–14 завершены (бэкенд + фронтенд + REST API + i18n + unified guide + per-agent roles + explicit conclude + MCP OAuth + agent-side retrieve + mobile adaptation).
...
pnpm test           # vitest run (242 tests)
...
- `TODO.md` — срезы 9–14 завершены.
- `ISSUES.md` — открытые задачи: callTool refactor, run composition, act gating, config.json per-folder, trace hierarchy, бэкапы SQLite, мобильная адаптация, guide паттерны.
