...
**Реализовано.** 241 тест, 30 MCP tools, typecheck clean.
Срезы 1–18 завершены (бэкенд + фронтенд + REST API + i18n + per-agent roles + explicit conclude + MCP OAuth + agent-side retrieve + mobile adaptation + monolithic guide + agent experience + A2 MCP Apps виджеты + artifact infra).
A1 sidebar dashboard отменён (CSP sandbox claude.ai блокирует connect-src).
Два ревью проведены и закрыты: system review (40/40) + MCP refactor review (16/16).
Production: `https://zs.docxi.org/mcp` (MCP) + SPA на том же домене.
Deploy: GitHub Actions → build frontend + build MCP apps → rsync → PM2 reload.
...
packages/
  ...
  scaffold/   — каноничные ambient .d.ts + tsconfig шаблоны + store.d.ts + guide (executor/architect)
...
Store (Collections + Notes), Agents, Agent Detail, Settings, Users, Help (13 статей, 4 категории).

**UI features:**
- Дизайн-токены из прототипа (`ui/`): dark/light themes, 4 accent colors, 3 density modes (comfortable/compact/mobile), 3 font families
...
- **Responsive (mobile):** 4 breakpoints (1024/860/768/640px). Sidebar→drawer, table→cards с mobile sort, split→tabs (code/events/coverage), help tree→accordion, tweaks→bottom sheet. CSS-driven via data-attributes

**i18n (мультиязычность):**
- `src/i18n/` — I18nProvider + useT() hook + useLocale()
- `en.ts` / `ru.ts` — ~180 UI строк каждый, полное покрытие
- Help docs: `public/docs/en/` и `public/docs/ru/` — 13 статей на каждом языке
...
```
start → commit → report(survey) → report(doubt) → check → checkpoint → conclude → status_transition(done)
```

Явные ops: `start` (при создании Instance), `commit`/`check` (пре-коммитмент), `conclude` (перед done transition).
...
30 MCP tools: 6 lifecycle + 2 commit/check + 2 trace records + 3 discovery + 4 CRUD +
8 store + 1 register + 1 abort + 1 guide + 1 retrieve + 1 dashboard

### MCP Apps (ext-apps) — A2 inline виджеты

8 inline виджетов рендерятся в чате (claude.ai, ChatGPT, VS Code) при вызове tools:
- `zs_start` → Trace Progress (script_ref, status, JSDoc, serverFunctions)
- `zs_report` → Report (операция + иконка, параметры из data)
- `zs_checkpoint` → Checkpoint (label, directive badge, data)
- `zs_sandbox` → Sandbox (fn(), args, result, verified)
- `zs_commit` → Commit (what/basis/verify/boundaries card)
- `zs_check` → Check (commit_seq, results, PASS/FAIL)
- `zs_retrieve` → Retrieve (query, provenance, trust badge)
- `zs_conclude` → Conclude (donut coverage, structured result)

Виджеты получают данные через `ontoolresult` (MCP), не через fetch — обходят CSP.
Сервер включает `_input` в tool result для fallback (ontoolinput не всегда срабатывает).
Build: `pnpm --filter @zobr/server build:apps` → Vite single-file HTML в `dist-apps/`.

### Artifact инфраструктура (серверная, готова)

- `/artifact/*` REST API: trace, events?since, instance, children (token-protected)
- `/artifact/ws/trace/:id` WebSocket: real-time event/status стрим
- Artifact token: jose HS256, scope check (invocation_id), 1h TTL
- SPA Trace Detail использует WS вместо polling (useTraceWs hook)
- `zs_dashboard` tool: возвращает visualization URLs + config + token
- **CSP блокирует connect-src в sandbox claude.ai** — A1 sidebar невозможен
- Инфраструктура ждёт починки CSP (Issues #40, #54 в anthropics/claude-ai-mcp)
...
- `@modelcontextprotocol/sdk` 1.29.0 (McpServer, StreamableHTTPServerTransport, OAuthServerProvider, mcpAuthRouter)
...
pnpm test           # vitest run (241 tests)
...
### Авто-генерация .sandbox.d.ts

При `zs_create`/`zs_update` и при старте сервера — для каждого скрипта с srv.ts
генерируется `{ref}.sandbox.d.ts` с `declare function` для публичных методов.
Monaco и tsc видят @sandbox функции без ручного declare.

## TODO / ISSUES

- `TODO.md` — срезы 15–18 завершены. Следующий: slice 19 (fullscreen A2 dashboard), slice 20 (HITL).
...
- **Monolithic guide** — два файла: `guide-executor.md` (базовый) + `guide-architect.md` (дельта). Отдаётся целиком при `zs_register` по роли. `zs_guide()` — для повторного чтения
...
- **Explicit conclude** — `conclude<T>(result: T)`: скрипт явно маппит каждое поле Result к переменным из предыдущих шагов. Sem-хэндлы через `as` cast
...
- **A1 pipeline отменён** — CSP sandbox claude.ai блокирует connect-src. Серверная инфраструктура (REST + WS + token) сохранена для SPA и будущего
- **.sandbox.d.ts авто-генерация** — extractClassInfo + generateMethodAmbient при create/update/startup
- **Fullscreen A2 dashboard** (планируется, slice 19) — conclude виджет с `availableDisplayModes: ["inline", "fullscreen"]`, полный trace timeline
